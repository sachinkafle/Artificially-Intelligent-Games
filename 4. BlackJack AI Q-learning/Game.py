...
#get it from our AI file
from AI import genAction, setQ
...
dealerLimit = 18 #Sum needed for dealer to stop hitting.  #real world : black jack
...
def hit(cardCount, isPlayer): 
   ...
   #Ace : 1 or 11 based on user, J, K, Q -> 10
    #usable Ace. unusable Ace
...
def aiStep(action, playersCard, dealersCards):
	if action == 1:
		playersCard.append(hit(len(playersCard), True)) #hit action
	else:
		#to make dealer reveal their card
...
def main():
    ...
    while running:
        ...
        if AI and not gameOver and not dealer:
        	...
        	#epsiolon greedy 
        	action = genAction(currentState, e, Q) #either Hit (1) or stick (0)
        	if action == 0:
        		dealer = True
        	...
        	if gameOver:
        		...
        		currentEpisode = [] #reset in environment in the gym module
        		
        	
        	#currentEpisode = [(state), action, reward]
